	     ...
	     unsigned long size)
{
	int idx = 0;
	__u32 startsect;
	__u32 sector_cnt = size/mydata->sect_size;
	...
	if (sector_cnt && disk_write(startsect, sector_cnt, buffer) < 0) {
		debug("Error writing data\n");
		return -1;
	}

	if (size % mydata->sect_size) {
		ALLOC_ALIGN_BUFFER(__u8, tmpbuf, mydata->sect_size, ARCH_DMA_MINALIGN);
	      ...
	      unsigned long maxsize)
{
	...
	do {
		...
		while (actsize < filesize) {
			...
			if ((newclust - 1) != endclust)/* not continous or linear, and save it cluster by cluster */
				goto getit;
	...
	unsigned long size)
{
	...
	mydata->fatbufnum = -1;
	mydata->fatbuf = memalign(ARCH_DMA_MINALIGN, FATBUFSIZE);  /* dw_mmc use dma!!! */
