...
//#define FAT_DBG(fmt, arg...)		printf(fmt, ##arg)
#define FAT_DBG(fmt, arg...)
 ...
 *                    
*/
static int write_pbr_fat16(block_dev_desc_t *dev_desc, disk_partition_t *info)
{
	struct bs *bs;
	struct bsbpb *bsbpb;
	struct bsx *bsx;
	...
	img = memalign(ARCH_DMA_MINALIGN, sizeof(u8)* 512);
	if(!img)
	{
	...
	setstr(bs->oem, "SAMSUNG", sizeof(bs->oem));

	bsbpb = (struct bsbpb *)(img + sizeof(struct bs));

	mk2(bsbpb->bps, 0x200);
	
	if(info->size < 0x10000)	/* small volume < 32MB 512B */
	{
		printf("Under 32M,using small volume!\n");
		mk1(bsbpb->spc, 1);
		mk2(bsbpb->sec, info->size);
		mk4(bsbpb->bsec, 0);
	}
	else if(info->size < 0x40000)	/* 128MB--> 2KB ok!*/
	{
		printf("Under 128M\n");
		mk1(bsbpb->spc, 4);
	}
	else if (info->size <= 0x80000)	/* 256MB --> 4KB ok!*/
	{ /* under 256M -> 2K */
		printf("Under 256M\n");
		mk1(bsbpb->spc, 8);
	}
	else if (info->size <= 0x100000)	/* 512MB --> 8KB ok!*/
	{
		printf("under 512M\n");
		mk1(bsbpb->spc, 16);
	}
	else if (info->size <= 0x200000)	/* 1GB --> 16KB ok!*/
	{
		printf("under 1GB,ok!\n");
		mk1(bsbpb->spc, 32);
	}
	else if (info->size <= 0x400000)	/* 2GB ---> 32KB not ok! */
	{
		printf("Under 2G, bug!fatwrite does not support it!\n");
		mk1(bsbpb->spc, 64);
		free(img);
		return -1;
	}
	...
	mk2(bsbpb->res, 0x1);			/* FAT16 is only 1 */
	mk1(bsbpb->nft, 2);			/*  2 FAT*/
	mk2(bsbpb->rde, 512);		/* root diretory entries */
	
	if(info->size > 0x10000)
	{
			mk2(bsbpb->sec, 0x0);
			mk4(bsbpb->bsec, (info->size/bsbpb->spc)*bsbpb->spc);
	}
	
	mk1(bsbpb->mid, 0xf8);	/* hd disk */

	bytes_per_sector =  bsbpb->bps[0]|bsbpb->bps[1] << 8;
	printf("sector: %d bytes\n", bytes_per_sector);
	fat_size = ((info->size/bsbpb->spc)*2 +  bytes_per_sector - 1)/bytes_per_sector;	/* it must be it*/
	mk2(bsbpb->spf, fat_size);	/* FAT 16 SIZE */
	
	mk2(bsbpb->spt, 0x0);
	mk2(bsbpb->hds, 0x0);
	mk4(bsbpb->hid, 0x0);
	
	bsx =(struct bsx *)(img + sizeof(struct bs) + sizeof(struct bsbpb));
	mk1(bsx->drv, 0x0);
	mk1(bsx->rsvd, 0x0);
	mk1(bsx->sig, 0x29);
	mk4(bsx->volid,0x3333);
	setstr(bsx->label, "NO NAME", sizeof(bsx->label));
	setstr(bsx->type, "FAT16", sizeof(bsx->type));
...
static int write_fat16(block_dev_desc_t *dev_desc, disk_partition_t *info, int fat_size)
{
	__u8 *dummy;
	__u8 *img;
	int i;
	...
	img = memalign(ARCH_DMA_MINALIGN,sizeof(__u8)*512);
	if(img == NULL) {
	...
	dummy = memalign(ARCH_DMA_MINALIGN, sizeof(__u8) * 8192);
	if(dummy == NULL) {
	...
	for (i = 0;i < erase_block_cnt + 10; i+=16) {
		if (dev_desc->block_write(dev_desc->dev, info->start +
			 1 + i, 16, (ulong *)dummy) != 16) {	/* reserved sector is only 1 */
...
static int write_pbr_fat32(block_dev_desc_t *dev_desc, disk_partition_t *info)
{
	struct bs *bs;
	struct bsbpb *bsbpb;
	...
	FAT_DBG("img_offset = 0x%x for bpb\n", img_offset);
	...
	FAT_DBG("img_offset = 0x%x for ext bpb\n", img_offset);
	...
	FAT_DBG("img_offset = 0x%x for bsx\n", img_offset);
	mk4(bsxbpb->bspf, (info->size / (spc * 128))); /* FAT Size 32, total clusters:  info->size /spc, 128 :32 B manager 128 clusters */
...
static int write_reserved_fat32(block_dev_desc_t *dev_desc, disk_partition_t *info)
{
	/* Set Reserved Region */
	__u8 *img;
	int i;
...
static int write_fat32(block_dev_desc_t *dev_desc, disk_partition_t *info, int
fat_size)
{
	__u8 *dummy;
	__u8 *img;
...
fat_format_device(block_dev_desc_t *dev_desc, int part_no)
{
	...
	if (!get_partition_info (dev_desc, part_no, &info)) {
		cur_part_info.start = info.start;
		cur_part = part_no;
	...
	int fat_size;
#if 0
	fat_size = write_pbr_fat32(dev_desc, &info);
	if(fat_size < 0)
		return -5;
	if(write_reserved_fat32(dev_desc, &info) < 0)
		return -6;
	if(write_fat32(dev_desc, &info, fat_size) < 0)
		return -7;
#else
	fat_size = write_pbr_fat16(dev_desc, &info);
	if(fat_size < 0)
		return -8;
	if(write_fat16(dev_desc, &info, fat_size) < 0)
		return -9;
#endif
