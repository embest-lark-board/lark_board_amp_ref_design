...
void linux_serial0_putc(int ch)
{
        u32 uart0_va_base = SOCFGA_UART0_VIRT_BASE;
        //u32 uart0_va_base = 0xffc02000;
        while ((*(volatile u32 *)(uart0_va_base+(5<<2)) & UART_LSR_THRE) == 0);
                *(volatile u32 *)uart0_va_base = ch;
}

void linux_serial0_puts(char *s)
{
	while(*s!='\0')
	{
		linux_serial0_putc(*s);
		s++;
	}
}
...
void __init l2x0_init(void __iomem *base, u32 aux_val, u32 aux_mask)
{
	...
	if (!(readl_relaxed(l2x0_base + L2X0_CTRL) & L2X0_CTRL_EN)) {
		...
		while(ACCESS_ONCE(*(u32 *)(AMP_SHARE_DMABUF_START + 0x100000)) != (('L'<<24) | ('O'<<16) | ('C'<<8)| 'K'))
		{
			i++;
			if(i%100 == 0)
				linux_serial0_puts("wait hand shake from bm....\r\n");
			msleep(10);
			if(i > 200) {
				linux_serial0_puts("wait hand shake from bm time out, boot any way....\r\n");
				boot_bm_failed = 1;
				break;
			}
		}
