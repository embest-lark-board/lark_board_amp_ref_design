...
s32 alt_dma_program_init(ALT_DMA_PROGRAM_t * pgm)
{
    ...
    u32 buffer = (u32)pgm->program;
    u32 offset = ((buffer + ALT_DMA_PROGRAM_CACHE_LINE_SIZE - 1) & ~(ALT_DMA_PROGRAM_CACHE_LINE_SIZE - 1)) - buffer;

    // It is safe to cast to u16 because the extra offset can only be up to
    // (ALT_DMA_PROGRAM_CACHE_LINE_SIZE - 1) or 31, which is within range of the
    // u16.
    pgm->buffer_start = (u16)offset;
                                           ...
                                           ALT_DMA_PROGRAM_REG_t reg, u32 val)
{
    ...
    buffer[0] = (u8)((val >>  0) & 0xff);
    buffer[1] = (u8)((val >>  8) & 0xff);
    buffer[2] = (u8)((val >> 16) & 0xff);
    buffer[3] = (u8)((val >> 24) & 0xff);
                                        ...
                                        ALT_DMA_PROGRAM_REG_t addr_reg, u16 val)
{
    ...
    u8 * buffer = pgm->program + pgm->buffer_start + pgm->code_size;
                                      ...
                                      ALT_DMA_SECURITY_t sec)
{
    ...
    buffer[1] = (u8)channel;
    buffer[2] = (u8)((val >>  0) & 0xff);
    buffer[3] = (u8)((val >>  8) & 0xff);
    buffer[4] = (u8)((val >> 16) & 0xff);
    buffer[5] = (u8)((val >> 24) & 0xff);
