...
function usage() {
    ...
    echo "usage: ${SELF} [-h] [-k f1,f2] [-p preloader -b bootloader] [-r rfs_dir [-m merge_dir]] [-o image]"
    echo "-k f1,f2,...   comma separated list of OS files  (i.e. uImage,dtb)"
    echo "-p preloader   preloader raw file (i.e. uboot-socfpga/u-boot-spl.bin)"
    echo "-b bootloader  bootloader raw file (i.e. uboot-socfpga/u-boot.img)"
    echo "-a bare metal  bare metal raw file (i.e  bm.bin)"
    echo "-f rbf for fpga rbf file for fpga  (i.e  bm.rbf)"
    echo "-r rfs_dir     location of the root file system files."
    echo "-o image       name of generated image file. Default size is ${DEF_IMAGE_NAME}"
    echo "-m merge_dir   copy files located in dir/ to rfs. Default is ${DEF_MERGE_DIR}"
    echo "-h             this message"
}
...
while [ $# -gt 0 ] ; do
    case $1 in
        ...
        -s) DEBUG=1 ;;
	-p) PRELOADER=$2 ; DO_PRELOADER=yes ; shift ;;
	-b) BOOTLOADER=$2 ; DO_PRELOADER=yes ; shift ;;
	-a) BM=$2 ; DO_PRELOADER=yes ; shift ;;
	-f) RBF=$2 ; DO_PRELOADER=yes ; shift ;;
...
if [ "X${DO_PRELOADER}" == "Xyes" ] ; then
	...
	BM=$(check_file f "no bare metal specified" ${BM})
	RBF=$(check_file f "no rbf for fpga" ${RBF})
  ...
  raw_copy_to_blk ${LOOP2} ${PRELOADER}
	if [ $? -ne 0 ] ; then
	...
	echo "${SELF}: info: copying BM image..."
	BM_OFF=1M
	echo "${BM}"
	dd if=${BM} of=${LOOP2} seek=${BM_OFF} bs=1 conv=fsync 
	if [ $? -ne 0 ] ; then
				echo "${SELF}: error: could not copy BM image to block ${LOOP2}"
				exit -1
	fi
	
	echo "${SELF}: info: copying RBF image..."
	RBF_OFF=2M
	echo "${RBF}"
	dd if=${RBF} of=${LOOP2} seek=${RBF_OFF} bs=1 conv=fsync
	if [ $? -ne 0 ] ; then
				echo "${SELF}: error: could not copy RBF image to block ${LOOP2}"
				exit -1
	fi
